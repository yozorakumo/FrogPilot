...
print('=== CLUTCH SIGNAL CAPTURE (cereal/messaging) ===', flush=True)
print('Capturing ALL CAN changes for 25 seconds...', flush=True)
print('Procedure: wait 5s, then press clutch 3 times (1 press/sec), then wait', flush=True)
print('', flush=True)
...
start = time.time()
print('[0.0s] === STARTING CAPTURE - Press clutch 3 times at 5s, 7s, 9s ===', flush=True)
...
# Also show current value of key IDs
