...
int UIEditModeManager::getSidebarOffsetX(const QString &name, bool sidebar_left, bool sidebar_right, int screen_width) {
  auto it = elements_.find(name);
  if (it == elements_.end()) {
    return 0;
  }
  ...
  it->cached_sidebar_offset_x = result;
  return result;
}
